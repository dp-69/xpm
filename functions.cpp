...
using namespace xpm::attribs;

namespace xpm
{
  dpl::vector3d angles_for_j_norm(const dpl::vector3d& dir) {
  ...
  vtkSmartPointer<vtkActor> CreateNodeActor(const pore_network_model& pnm, vtkLookupTable* lut) {
    ...
    for (pnm_idx i = 0, count = pnm.node_count_; i < count; ++i) {
      ...
      color_array->InsertNextTuple1(pnm.node_[r_ins][i]);
    }
